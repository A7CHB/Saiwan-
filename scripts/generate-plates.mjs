...
/**
 * The home hero, cut into planes for the 3D stage.
 *
 * The hero is not one flat photograph parallaxed by hand — it is four
 * transparent planes standing at different distances inside a perspective
 * container, so the separation you see when you scroll or move the pointer is
 * the projection doing the work rather than four hand-tuned speeds. Each plane
 * is generated at the same viewBox and overlaid; only the sky is opaque.
 */
function heroPlanes(mood, seed) {
  const m = MOODS[mood];
  // Square, and composed from the centre out. The planes are cropped to
  // whatever shape the viewport is — a wide desktop takes the top and bottom
  // off, a phone takes the sides off — so the subject has to survive both.
  // A landscape plate loses its subject entirely on a phone.
  const width = 2000;
  const height = 2000;
  ...
  const defs = `
    ...
    <linearGradient id="fore" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="${m.ground}" stop-opacity="0"/>
      <stop offset="55%" stop-color="${m.ground}" stop-opacity="0.5"/>
      <stop offset="100%" stop-color="${m.ground}" stop-opacity="0.92"/>
    </linearGradient>
    <linearGradient id="haze" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="${m.sky[1]}" stop-opacity="0"/>
      <stop offset="70%" stop-color="${m.sky[1]}" stop-opacity="0.34"/>
      <stop offset="100%" stop-color="${m.sky[1]}" stop-opacity="0"/>
    </linearGradient>
    <filter id="soft"><feGaussianBlur stdDeviation="${(width * 0.045).toFixed(1)}"/></filter>
    <filter id="far"><feGaussianBlur stdDeviation="7"/></filter>
    <filter id="grain"><feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="3"/></filter>`;
  ...
  // 1. Sky — the only opaque plane, furthest away.
  const sky = wrap(`
  <rect width="${width}" height="${height}" fill="url(#sky)"/>
  <circle cx="${width * 0.72}" cy="${horizon * 0.34}" r="${width * 0.08}" fill="${m.sun}" opacity="0.6" filter="url(#soft)"/>
  <rect x="0" y="${horizon - height * 0.2}" width="${width}" height="${height * 0.26}" fill="url(#haze)"/>
  <rect x="0" y="0" width="${width}" height="${height}" filter="url(#grain)" opacity="0.05"/>`);

  // 2. Far — a blurred skyline standing well behind the subject.
  let x = -width * 0.05;
  let index = 0;
  const skyline = [];
  while (x < width * 1.05) {
    const w = width * (0.07 + random() * 0.15);
    const h = height * (0.05 + random() * 0.15);
    skyline.push(
      `<rect x="${x.toFixed(1)}" y="${(horizon - h).toFixed(1)}" width="${w.toFixed(1)}" height="${(h + 4).toFixed(1)}" fill="${m.ground}" opacity="${(0.16 + (index % 3) * 0.05).toFixed(2)}"/>`,
    );
    x += w * (0.82 + random() * 0.5);
  ...
  const far = wrap(`<g filter="url(#far)">${skyline.join("")}
  ${canopy(width * 0.12, horizon + height * 0.03, height * 0.4, height * 0.13, m.ground, 0.4, m.sun)}
  ${canopy(width * 0.86, horizon + height * 0.02, height * 0.36, height * 0.12, m.ground, 0.34, m.sun)}</g>`);

  // 3. Mid — the subject. Two canopies, the near one large and centre-right so
  //    it survives every crop from a phone to a wide desktop.
  const mid = wrap(`
  ${canopy(width * 0.26, horizon + height * 0.05, height * 0.44, height * 0.15, m.ground, 0.58, m.sun)}
  ${canopy(width * 0.52, horizon + height * 0.11, height * 0.62, height * 0.21, m.ground, 0.94, m.sun)}`);

  // 4. Near — the ground the viewer stands on. A gradient, not a slab: this
  //    plane is closest to the camera and an opaque edge on it would read as a
  //    black bar rather than as foreground.
  const near = wrap(`
  <rect x="0" y="${horizon + height * 0.22}" width="${width}" height="${height * 0.6}" fill="url(#fore)"/>
  <rect x="0" y="${horizon + height * 0.22}" width="${width}" height="2" fill="${m.accent}" opacity="0.18"/>`);

  return { sky, far, mid, near };
}

const HERO_PLANES = heroPlanes("golden", 4241);
for (const [plane, svg] of Object.entries(HERO_PLANES)) {
  writeFileSync(join(OUT, `hero-plane-${plane}.svg`), svg.trim());
}
