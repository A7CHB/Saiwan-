...
/**
 * Environment plates for the home showroom.
 *
 * Deliberately empty. The showroom stands one umbrella in front of these and
 * changes the world behind it, so a canopy drawn into the plate would give the
 * scene two umbrellas. The horizon sits high and the centre foreground is left
 * clear: that is where the umbrella stands.
 *
 * These are placeholders for photography, and the only thing that has to hold
 * is the geometry — horizon height and an empty centre — so a real photograph
 * can replace one without the umbrella suddenly floating.
 */
function environment({ mood, water = false, skyline = "low", planting = 0, seed }) {
  const m = MOODS[mood];
  const size = 2000;
  ...
  layers.push(
    `<circle cx="${size * 0.72}" cy="${horizon * 0.36}" r="${size * 0.08}" fill="${m.sun}" opacity="0.55" filter="url(#soft)"/>`,
  );

  // Architecture. `tall` reads as a city skyline behind a rooftop; `low` as the
  // wall of a villa or a garden boundary.
  let x = -size * 0.05;
  let index = 0;
  while (x < size * 1.05) {
    const w = size * (skyline === "tall" ? 0.05 + random() * 0.1 : 0.09 + random() * 0.18);
    const h = size * (skyline === "tall" ? 0.08 + random() * 0.28 : 0.04 + random() * 0.12);
    layers.push(
      `<rect x="${x.toFixed(1)}" y="${(horizon - h).toFixed(1)}" width="${w.toFixed(1)}" height="${(h + 4).toFixed(1)}" fill="${m.ground}" opacity="${(0.3 + (index % 3) * 0.08).toFixed(2)}"/>`,
    );
    x += w * (0.86 + random() * 0.45);
    index += 1;
  }

  layers.push(
    `<rect x="0" y="${horizon}" width="${size}" height="${size - horizon}" fill="url(#ground)"/>`,
    // The line where the ground meets the architecture. Without it the plate
    // reads as haze rather than as a place with a floor.
    `<rect x="0" y="${horizon.toFixed(1)}" width="${size}" height="3" fill="${m.accent}" opacity="0.35"/>`,
  );

  // Low furniture along the edges — a bench, a lounger, a table. Abstract, but
  // enough to give the eye a scale next to the umbrella.
  for (let i = 0; i < 3; i++) {
    const side = i % 2 === 0 ? 0.04 + random() * 0.12 : 0.8 + random() * 0.14;
    const w = size * (0.1 + random() * 0.08);
    const h = size * (0.02 + random() * 0.02);
    const y = horizon + size * (0.08 + random() * 0.16);
    layers.push(
      `<rect x="${(size * side).toFixed(1)}" y="${y.toFixed(1)}" width="${w.toFixed(1)}" height="${h.toFixed(1)}" rx="${(h * 0.3).toFixed(1)}" fill="${m.ground}" opacity="0.55"/>`,
    );
  }

  if (water) {
    const top = horizon + (size - horizon) * 0.3;
    layers.push(`<rect x="0" y="${top}" width="${size}" height="${size - top}" fill="url(#water)"/>`);
    for (let i = 0; i < 7; i++) {
      const y = top + (size - top) * (0.1 + i * 0.13);
      layers.push(
        `<rect x="${(size * random() * 0.35).toFixed(1)}" y="${y.toFixed(1)}" width="${(size * (0.2 + random() * 0.5)).toFixed(1)}" height="2" fill="${m.accent}" opacity="0.2"/>`,
      );
    }
  }

  // Planting sits at the edges only, so the centre stays clear for the umbrella.
  for (let i = 0; i < planting; i++) {
    const side = i % 2 === 0 ? 0.06 + random() * 0.16 : 0.78 + random() * 0.16;
    const cx = size * side;
    const cy = horizon + size * (0.02 + random() * 0.1);
    const r = size * (0.06 + random() * 0.07);
    layers.push(
      `<ellipse cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" rx="${r.toFixed(1)}" ry="${(r * 1.25).toFixed(1)}" fill="${m.ground}" opacity="0.72"/>`,
    );
  }

  layers.push(
    `<rect x="0" y="${horizon - size * 0.14}" width="${size}" height="${size * 0.2}" fill="url(#haze)"/>`,
    `<rect x="0" y="0" width="${size}" height="${size}" fill="url(#sweep)"/>`,
    `<rect x="0" y="0" width="${size}" height="${size}" filter="url(#grain)" opacity="0.05"/>`,
  );
  ...
  <defs>
    ...
    <linearGradient id="ground" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="${m.ground}" stop-opacity="0.5"/>
      <stop offset="100%" stop-color="${m.ground}" stop-opacity="0.92"/>
    </linearGradient>
    <linearGradient id="water" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="${m.sky[1]}" stop-opacity="0.85"/>
      <stop offset="100%" stop-color="${m.sky[2]}" stop-opacity="0.95"/>
    ...
    <linearGradient id="sweep" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="${m.accent}" stop-opacity="0.14"/>
      <stop offset="50%" stop-color="${m.accent}" stop-opacity="0"/>
      <stop offset="100%" stop-color="${m.ground}" stop-opacity="0.2"/>
    </linearGradient>
    <filter id="soft"><feGaussianBlur stdDeviation="${(size * 0.045).toFixed(1)}"/></filter>
    <filter id="grain"><feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="3"/></filter>
  </defs>
...
// name → environment options. One per showroom scene.
const ENVIRONMENTS = [
  ["env-villa", { mood: "stone", water: true, skyline: "low", planting: 2 }],
  ["env-resort", { mood: "midday", water: true, skyline: "low", planting: 4 }],
  ["env-dining", { mood: "golden", water: false, skyline: "low", planting: 2 }],
  ["env-rooftop", { mood: "dusk", water: false, skyline: "tall", planting: 2 }],
  ["env-garden", { mood: "olive", water: false, skyline: "low", planting: 6 }],
];

for (const [name, options] of ENVIRONMENTS) {
  seed += 7919;
  writeFileSync(join(OUT, `${name}.svg`), environment({ ...options, seed }).trim());
}
